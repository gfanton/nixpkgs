# from https://github.com/malob/nixpkgs/blob/b99ea238d7471cfcd2438a47063af58346400b62/home/modules/programs/kitty/extras.nix
...
let
  ...
  cfg = config.programs.mykitty.extras;
...
in {
  ...
  options.programs.mykitty.extras = {
    colors = {
      enable = mkOption {
        type = types.bool;
        default = false;
  ...
  config = mkIf config.programs.mykitty.enable {
    ...
    programs.mykitty.settings = optionalAttrs cfg.colors.enable (
