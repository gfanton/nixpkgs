...
#+begin_src emacs-lisp
  ...
  (unless (display-graphic-p)
    ...
    (emacs-nix-log "Mouse scroll support enabled for terminal"))
  ...
  ;; Replace yes/no prompts with y/n (no need to press enter)
...
#+begin_src emacs-lisp
  ;; Disable automatic buffer refreshing when files change on disk
  (setq global-auto-revert-mode nil)
  (setq auto-revert-mode nil)
  (setq auto-revert-interval 5)  ; If you ever enable it manually, check every 5 seconds

  ;; Disable dired auto-revert specifically
  (setq dired-auto-revert-buffer nil)

  ;; Ensure global mode is disabled
  (global-auto-revert-mode -1)

  (emacs-nix-log "Auto-revert disabled - buffers will not automatically refresh")
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; Set appearance for external callers (tmux hooks, scripts)
  (defun my/set-appearance (mode)
    "Set appearance to MODE (`dark' or `light', as symbol or string)."
    (let ((flavor (pcase (if (stringp mode) (intern mode) mode)
                    (`dark 'macchiato)
                    (`light 'latte)
  ...
  (defun my/toggle-theme ()
    "Toggle between light and dark catppuccin theme flavors."
    (interactive)
    (my/set-appearance
     (if (eq catppuccin-flavor 'macchiato) 'light 'dark)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; Unbind comma from evil-repeat-find-char-reverse
  ;; This prevents "no previous search" errors when using comma as major mode leader
  (with-eval-after-load 'evil
    (define-key evil-normal-state-map (kbd ",") nil)
    (define-key evil-visual-state-map (kbd ",") nil)
    (emacs-nix-log "Evil comma key unbound to prevent conflicts with major mode leader"))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package general
    ...
    (my/leader-keys
      ...
      "gl" '(magit-live-diff-mode :which-key "toggle live diff")
      "gc" '(magit-comment-transient :which-key "comments")
      ...
      "sf" '(my/consult-ripgrep-directory :which-key "search directory")
  ...
  (defun my/consult-ripgrep-directory (&optional dir initial)
    "Search in a specific directory with consult-ripgrep."
    (interactive "DDirectory: ")
    (let ((search-dir (or dir (read-directory-name "Search directory: " default-directory))))
      (consult-ripgrep search-dir initial)))
...
#+begin_src emacs-lisp
  ...
  (use-package go-mode
    ...
    :config
    (defun my/go-mode-before-save-hooks ()
      "Setup before-save hooks for Go mode."
      (add-hook 'before-save-hook #'lsp-format-buffer nil t)
      (add-hook 'before-save-hook #'lsp-organize-imports nil t))
    (add-hook 'go-mode-hook #'my/go-mode-before-save-hooks))
...
#+begin_src emacs-lisp
  (use-package magit
    ...
    ;; Add SSH commit signing toggle to the commit transient
    (transient-append-suffix 'magit-commit "-C"
      '("-S" "Sign commit" "--gpg-sign")))

  (use-package magit-live-diff
    :commands (magit-live-diff-mode)
    :init
    ;; Enable automatically when magit loads
    (with-eval-after-load 'magit
      (magit-live-diff-mode 1)))

  (use-package magit-comment
    :commands (magit-comment-mode magit-comment-transient)
    :init
    ;; Enable automatically when magit loads
    (with-eval-after-load 'magit
      (magit-comment-mode 1)
      ;; Require transient to register with magit-dispatch
      (require 'magit-comment-transient)))

  (use-package git-timemachine
    :after magit
    :config
    (emacs-nix-log "Git timemachine configured"))
...
#+begin_src emacs-lisp
  ...
  ;; ws-butler disabled - SSL cert issue with savannah.gnu.org prevents Nix from building it
  ...
  (use-package vundo
    :bind (("C-x u" . vundo))
    :config
    ;; Better default settings for terminal use
    (setq vundo-glyph-alist vundo-unicode-symbols)
    (emacs-nix-log "Vundo visual undo tree configured"))
#+end_src
...
#+begin_src emacs-lisp
  ;; Window movement with M-hjkl (avoids shadowing C-h help and C-k kill-line)
  (global-set-key (kbd "M-h") 'windmove-left)
  (global-set-key (kbd "M-j") 'windmove-down)
  (global-set-key (kbd "M-k") 'windmove-up)
  (global-set-key (kbd "M-l") 'windmove-right)

  ;; Enable windmove with shift+arrow keys as fallback
...
#+begin_src emacs-lisp
  ...
  (defvar my/auto-delete-trailing-whitespace t
    "Automatically delete trailing whitespace on save.")

  (defun my/toggle-auto-delete-trailing-whitespace ()
    "Toggle the automatic deletion of trailing whitespace."
    (interactive)
    (setq my/auto-delete-trailing-whitespace (not my/auto-delete-trailing-whitespace))
    (if my/auto-delete-trailing-whitespace
  ...
  (defun my/maybe-delete-trailing-whitespace ()
    "Delete trailing whitespace if `my/auto-delete-trailing-whitespace' is non-nil."
    (when my/auto-delete-trailing-whitespace
